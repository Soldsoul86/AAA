...
again lives in the [Soldsoul86/AAA](https://github.com/Soldsoul86/AAA)
...
## Every release of again after that

Tags are prefixed per tool, since one repo holds four release pipelines:

```
git tag again-v0.1.0
git push origin again-v0.1.0
```

This triggers `.github/workflows/release-again.yml` only — the other three
tools' workflows won't run. Within a few minutes:
- A GitHub Release tagged `again-v0.1.0` exists with binaries for macOS
  (Intel + Apple Silicon), Linux (amd64 + arm64), and Windows
- `checksums.txt` is published alongside them
- `brew install Soldsoul86/tap/again` works
- `install.sh` and `go install github.com/Soldsoul86/AAA/again@latest` both resolve to this version
...
`again/dist/`, without needing a git tag or pushing anywhere.
