...
ctxmeter lives in the [Soldsoul86/AAA](https://github.com/Soldsoul86/AAA)
...
## Every release of ctxmeter after that

Tags are prefixed per tool, since one repo holds four release pipelines:

```
git tag ctxmeter-v0.1.0
git push origin ctxmeter-v0.1.0
```

This triggers `.github/workflows/release-ctxmeter.yml` only — the other three
tools' workflows won't run. Within a few minutes:
- A GitHub Release tagged `ctxmeter-v0.1.0` exists with binaries for macOS
  (Intel + Apple Silicon), Linux (amd64 + arm64), and Windows
- `checksums.txt` is published alongside them
- `brew install Soldsoul86/tap/ctxmeter` works
- `install.sh` and `go install github.com/Soldsoul86/AAA/ctxmeter@latest` both resolve to this version
...
`ctxmeter/dist/`, without needing a git tag or pushing anywhere.
