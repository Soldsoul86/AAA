...
// locateClaudeCodeSession is best-effort: it relies on an observed path
// pattern (~/.claude/projects/<cwd with / replaced by ->/*.jsonl), not a
// documented, guaranteed API. If it doesn't find anything, use -file instead.
func locateClaudeCodeSession() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("no project directory found at %s (this auto-detection is best-effort — use -file <path> if you know the right file)", dir)
	}
