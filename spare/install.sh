#!/bin/sh
# Installs the latest release of spare from GitHub Releases.
# Usage: curl -fsSL https://raw.githubusercontent.com/Soldsoul86/AAA/main/spare/install.sh | sh
#
# This repo holds several tools. Each tool's release is uniquely tagged
# "vX.Y.Z+spare" (semver build metadata, so it can never collide with
...
# Full release tag, e.g. "v0.1.0+spare"
...
# used in the archive filename: "v0.1.0+spare" -> "0.1.0"
...
echo
echo "Run '$BINARY init' next, then open a new terminal — installing the"
echo "binary alone doesn't activate anything yet."
